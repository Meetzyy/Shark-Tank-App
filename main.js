...
searchByButton.addEventListener('click', () => {
    ...
    let searchType = searchBySelect.value;
    
    if(searchType === "title") {
        let filteredData = product_data.filter((el) => el.title && el.title.toLowerCase().includes(searchValue));
        displayData(filteredData);
    } else if (searchType === "founder") {
        let filteredData = product_data.filter((el) => el.founder && el.founder.toLowerCase().includes(searchValue));
        displayData(filteredData);
    } else {
        alert("Please select what to search by (Title or Founder)");
    }
});
...
pitchCreateBtn.addEventListener('click',(e)=>{
    ...
    .then((res)=>res.json())
    .then(data=>{
        console.log('Pitch added Sucessfully:',data);
        fetchData(); // refresh data
    })
    .catch((err)=>console.log(err))
})
...
updatePitchBtn.addEventListener('click',()=>{
   let updatePitchObj = {
    ...
    fetch(`${API_BASE_URL}/pitches/${updatePitchObj.id}`,{
        method:'PUT',
        headers : {
            'Content-Type': 'application/json',
        },
